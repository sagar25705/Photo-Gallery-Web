...
//     queryFn: async () => {
//       const { data } = await supabase.from("comments").select("*").eq("photo_id", id).order("created_at", { ascending: false });
//       return data ?? [];
//     },
//   });
...
function PhotoDetail() {
  ...
  if (!photo) {
    return (
      <Layout>
        <div className="p-10 text-center">Photo not found.</div>
      </Layout>
    );
  }
...
if (profile?.is_banned) {
  ...
  return;
    ...
    if (saved) {
      ...
      await supabase.from("saved_photos").insert({
        user_id: user.id,
        photo_id: id,
      });
  ...
  const download = async () => {
    try {
      ...
      await supabase
        .from("photos")
        .update({
          downloads_count: (photo.downloads_count ?? 0) + 1,
        })
        .eq("id", id);
...
if (profile?.is_banned) {
  ...
  const author = photo?.profiles as AuthorProfile | null;
            ...
            <div className="glass-strong rounded-3xl p-6">
              ...
              <div className="grid grid-cols-4 gap-2 mt-5">
                ...
                <ActionBtn
                  ...
                  label={String(photo.likes_count)}
                  onClick={toggleLike}
                />

                <ActionBtn
                  icon={<Download />}
                  label={String(photo.downloads_count)}
                  onClick={download}
                />
              ...
              {(photo.tags ?? []).length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {(photo.tags ?? []).map((t: string) => (
                    <span
                      key={t}
                      className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/30"
                    >
                      {t}
                    </span>
                  ))}
...
function ActionBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="glass rounded-xl py-3 flex flex-col items-center gap-1 hover:bg-primary/10 transition-colors text-xs"
    >
      {icon}
      <span>{label}</span>
    </button>
